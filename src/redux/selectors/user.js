
export const passwordWrongState$ = state => state.user.passwordWrong;

export const emailExistState$ = state => state.user.emailExist;

export const createUserState$ = state => state.user.createUser;

export const otpState$ = state => state.user.otp;

export const otpErrorState$ = state => state.user.otpError;

export const resetPasswordState$ = state => state.user.resetPassword;

export const resetPasswordErrorState$ = state => state.user.resetPasswordError;

export const infoUserState$ = state => state.user.infoUser;

export const userOnlineState$ = state => state.user.userOnline;

export const infoChangePasswordState$ = state => state.user.infoChangePassword;

export const isLoginState$ = state => state.user.isLogin;

